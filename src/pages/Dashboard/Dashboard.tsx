...
import { getAppointmentByStoreId, getAppointmentRevenue } from "../../services/AppointmentServices";
...
const Dashboard = () => {
  ...
  const [appointmentPercentageCanceled, setAppointmentPercentageCanceled] = useState<number>(0);
    ...
    const fetchData = async () => {
      try {
        ...
        if (responseRevenue && Array.isArray(responseRevenue)) {
          ...
          if (responseAppointmentCount && Array.isArray(responseAppointmentCount)) {
            ...
            const percentageCanceled = (canceledAppointments.length / totalAppointments) * 100;
  ...
  return (
    <ContainerPage style={{ height: "100%", width: "99.4%" }}>
      ...
      <S.DashboardContainer>
        ...
        <Col sm={12} xl={4}>
          <DashboardCard title="Agendamentos" value={appointmentCount.toString()} icon="arrowUp" />
        </Col>
        <Col sm={12} xl={4}>
          <DashboardCard title="Cancelamento" value={`${appointmentPercentageCanceled.toString()}%`} icon="arrowDown" />
        </Col>
      </S.DashboardContainer>
