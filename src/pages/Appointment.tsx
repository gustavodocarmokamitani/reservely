...
export function Appointment() {
  ...
  const [appointmentTime, setAppointmentTime] = useState<SelectOption | null>(null);
  ...
  const validateAppointment = (
    ...
    const validationRules = [
      { condition: !employee?.value, message: "Por favor, selecione um funcionário." },
      { condition: !client?.value, message: "Por favor, selecione um cliente." },
      { condition: !service || service.length === 0 || service[0].value === 0, message: "Por favor, selecione pelo menos um serviço." },
      { condition: !appointmentTime?.value, message: "Por favor, selecione um horário." },
      { condition: !appointmentDate, message: "Por favor, selecione uma data para o agendamento." },
    ];
  ...
  const handleSubmit = async () => {
    try {
      if (!validateAppointment(employee, client, service, appointmentTime, appointmentDate, enqueueSnackbar)) {
        return;
      }
      ...
      agendamentoData.setHours(parseInt(horarioHoras, 10), parseInt(horarioMinutos, 10));
  ...
  return (
    <ContainerPage style={{ height: "100vh" }}>
      ...
      <S.AppointmentContainer>
        ...
        <S.AppointmentContent>
          <p>Serviço</p>
          <ServiceSelect value={service?.map((s) => s.value) || undefined} setService={setService} selectedEmployee={selectedEmployee} />
        </S.AppointmentContent>
        <S.AppointmentContent>
          <p>Horário</p>
          <TimeSelect value={appointmentTime?.value} setAppointmentTime={setAppointmentTime} />
        </S.AppointmentContent>
      </S.AppointmentContainer>
      <S.AppointmentContainer>
        <S.AppointmentContent>
